...
void Gramar::deal_with_BetaLambdaTau(MeshDataLMC& data, bool sample,
                                     bool sample_beta, bool sample_lambda, bool sample_tau){
  sample_hmc_BetaLambdaTau(sample, sample_beta, sample_lambda, sample_tau);
}


arma::vec Gramar::sample_BetaLambda_row(bool sample, int j, const arma::mat& rnorm_precalc){
  // build W
  ...
  arma::mat WWj = mgp.w.submat(ix_by_q_a(j), subcols); // acts as X //*********
...
void Gramar::sample_hmc_BetaLambdaTau(bool sample, bool sample_beta, bool sample_lambda, bool sample_tau){
  if(verbose & debug){
  ...
  Rcpp::RNGScope scope;
  arma::mat rnorm_precalc = mrstdnorm(q, mgp.k+p);
...
#pragma omp parallel for 
  ...
  for(unsigned int j=0; j<q; j++){
    ...
    if(familyid(j) == 0){
      ...
      arma::vec sampled;
        ...
        if((lambda_hmc_started(j) == 0) & (lambda_hmc_adapt.at(j).i == 10)){
          ...
          double lambda_eps = find_reasonable_stepsize(-1, mgp, 
                                                       curLrow, lambda_node.at(j), rnorm_row);
        ...
        if(which_hmc == 1){
          // mala
          sampled = mala_cpp(curLrow, -1, mgp, 
                             lambda_node.at(j), lambda_hmc_adapt.at(j), 
                             rnorm_row, lambda_runif(j), true, debug);
        }
        
        if((which_hmc == 3) || (which_hmc == 4)){
          // some form of manifold mala
          sampled = manifmala_cpp(curLrow, -1, mgp, 
                                  lambda_node.at(j), lambda_hmc_adapt.at(j), 
    ...
    LambdaHw.col(j) = mgp.w * arma::trans(Lambda.row(j));
