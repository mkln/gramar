...
Rcpp::List gramar_mcmc(
  ...
  Gramar msp(y, family,
            X, coords, X, axis_partition, k,
              
                start_w, beta, start_lambda, lambda_mask, 
                start_theta, 
                1.0/tausq, 
                beta_Vi, tausq_ab,
  ...
  arma::vec param = arma::vectorise(msp.mgp.param_data.theta);
  ...
  for(int i=0; i<mcmc_keep; i++){
    ...
    v_mcmc[iname] = Rcpp::wrap(arma::zeros(msp.mgp.w.n_rows, k));
    yhat_mcmc[iname] = Rcpp::wrap(arma::zeros(msp.y.n_rows, q)); 
    if(!low_mem){
      w_mcmc[iname] = Rcpp::wrap(arma::zeros(msp.mgp.w.n_rows, q));
  ...
  if(mcmc > 0){
    acceptable = msp.mgp.get_mgplogdens_comps(msp.mgp.param_data );
    acceptable = msp.mgp.get_mgplogdens_comps(msp.mgp.alter_data );
  }
  
  double current_loglik = tempr*msp.mgp.param_data.loglik_w;
  if(verbose & debug){
  ...
  try {
    ...
    for(m=0; (m<mcmc) & (!interrupted); m++){
      ...
      if(arma::any(family > 0)){
      //if(false){
        if(sample_lambda+sample_beta){
          start = std::chrono::steady_clock::now();
          msp.deal_with_BetaLambdaTau(msp.mgp.param_data, true, sample_beta, sample_lambda, sample_tausq); // true = sample
          end = std::chrono::steady_clock::now();
          if(verbose_mcmc & verbose){
        ...
        if(sample_lambda+sample_beta){
          start = std::chrono::steady_clock::now();
          msp.deal_with_BetaLambdaTau(msp.mgp.param_data, true, sample_beta, sample_lambda, false); // true = sample
          end = std::chrono::steady_clock::now();
          if(verbose_mcmc & verbose){
            Rcpp::Rcout << "[BetaLambdaTau] " 
                        << std::chrono::duration_cast<std::chrono::microseconds>(end - start).count() << "us.\n"; 
      ...
      if(sample_tausq || sample_beta || sample_w || sample_lambda){
        start = std::chrono::steady_clock::now();
        msp.mgp.logpost_refresh_after_gibbs(msp.mgp.param_data);
        end = std::chrono::steady_clock::now();
        if(verbose_mcmc & verbose){
      ...
      arma::mat lambda_transf_back = msp.Lambda;// * 
        //ps_back(msp.mgp.param_data.theta, d, 1, //msp.matern.twonu, 
        //        use_ps);
      ...
      if(mx >= 0){
        ...
        theta_mcmc.slice(w_saved) = msp.mgp.param_data.theta;
        ...
        wllsave(w_saved) = msp.mgp.param_data.loglik_w;
        w_saved++;
        
        if(mx % mcmc_thin == 0){
          std::string iname = std::to_string(mcmc_saved);
          
          v_mcmc[iname] = Rcpp::wrap(msp.mgp.w //* ps_forward(msp.mgp.param_data.theta, 
                                              //       d, 1, //msp.matern.twonu, 
                                              //       use_ps) 
                                       );
      ...
      if((m>0) & (mcmc > 100)){
        ...
        if(print_condition){
          ...
          tick_mcmc = std::chrono::steady_clock::now();
          if(verbose & debug){
            Rprintf("  p(w|theta) = %.2f    p(y|...) = %.2f  \n ", msp.mgp.param_data.loglik_w, msp.logpost);
          }
