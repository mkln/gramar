...
arma::uvec turbocolthreshold(const arma::vec& col1, const arma::vec& thresholds){
...
/*
//[[Rcpp::export]]
arma::umat part_axis_parallel(const arma::mat& coords, const arma::vec& Mv, int n_threads, bool verbose=false){
  if(verbose){
    Rcpp::Rcout << "  Axis-parallel partitioning... ";
  }
  arma::umat resultmat = arma::zeros(arma::size(coords));
...
//[[Rcpp::export]]
arma::umat part_axis_parallel_fixed(const arma::mat& coords, const arma::field<arma::vec>& thresholds, int n_threads){
  arma::umat resultmat = arma::zeros(arma::size(coords));
