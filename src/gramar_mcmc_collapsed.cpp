...
Rcpp::List mgp_precision(const arma::mat& coords, 
  ...
  MGP mgp(coords, xcoords_in, 
          axis_partition, na_mat, false,
          theta, false, metrop_theta_sd_btm,
          metrop_theta_bounds, false, 2, verbose, debug);
...
Rcpp::List gramar_mcmc_collapsed(
  ...
  if(mcmc > 0){
    ...
    msp.mgp.solver_initialize();
    //msp.mgp.new_precision_matrix_direct(msp.mgp.param_data);
    //msp.mgp.new_precision_matrix_direct(msp.mgp.alter_data);
  }
