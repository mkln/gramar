#ifndef MGP_UTILS_LMC
#define MGP_UTILS_LMC
...
inline T field_v_concatm(arma::field<T> const& fieldmats){
  ...
  for(int j=0; j<fieldmats.n_elem; j++){
    if(fieldmats(j).n_rows>0){
      //Rcpp::Rcout << "from " << ddims(j) << " to " << ddims(j+1)-1 << endl;
...
inline void add_grd_smu_parents_ptr_(arma::mat& result, 
                                     const arma::mat& x,
                                 const arma::cube* condprec,
                                 const arma::cube* cmk,
                                 const arma::mat& wparents){
  ...
  for(int i=0; i<n_blocks; i++){
    //result.rows(blockdims(i), blockdims(i+1)-1) +=
    result.rows(i*bsize, (i+1)*bsize-1) +=
      (*condprec).slice(i) * (x.col(i) - (*cmk).slice(i) * wparents.col(i));
  }
}
...
inline arma::cube AKuT_x_R_ptr(arma::cube& result, const arma::cube& x, const arma::cube* y){ 
  result = arma::zeros(x.n_cols, (*y).n_cols, x.n_slices);
  for(unsigned int i=0; i<x.n_slices; i++){
    result.slice(i) = arma::trans(x.slice(i)) * (*y).slice(i); 
  }
  return result;
...
inline double AK_vec_outer_ptr(const arma::cube* x, const arma::mat& y){ 
  double result = 0;
  int n_blocks = (*x).n_slices;
  for(int i=0; i<n_blocks; i++){
    result += arma::conv_to<double>::from(
      arma::trans(y.col(i)) * (*x).slice(i) * y.col(i));
  }
  return result;
}
